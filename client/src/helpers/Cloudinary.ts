...
export type ErrorState = Record<
  ...
  string
>;
export type ErrorStateSeller = Record<
  | "nombreUrl"
  | "nombreNegocio"
  | "imageLogo"
  | "categorias"
...
export const upLoadImageSeller =
  (
    input: InputStateSeller,
    setLoading: Dispatch<SetStateAction<boolean>>,
    setInput: Dispatch<SetStateAction<InputStateSeller>>,
    setErr: Dispatch<SetStateAction<ErrorStateSeller>>
  ) =>
  async (e: any) => {
    e.preventDefault();
    const files = e.target.files;
